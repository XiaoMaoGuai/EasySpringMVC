...
import com.xiaomaoguai.lib.core.utils.XmgStringUtils;

/*
 * @Description :
 * @FileName: MessageResource.java
 * @Author :WeiHui.Zhang
 * @Data : 2014年11月9日 下午1:53:33
 * @Version:1.0.0
 */
public class MessageResource {
	...
	private void loadResourceBundles() {
		ServiceLoader<ResourceBundleProvider> providers = ServiceLoader
				.load(ResourceBundleProvider.class);
	...
	private ResourceBundle getBundle(String bundleName,
			ClassLoader classLoader, Locale targetLocale, String key) {
		...
		try {
			...
			logger.debug("no i18n resource for locale[" + targetLocale + "].",
					e);
			return null;
		}
		if (bundle != null) {
	...
	public synchronized String getMessage(String key, Locale locale,
		...
		for (String bundleName : this.bundleNames) {
			// 资源是否已经加载
			if (this.loadedBundleKeys.contains(new LoadedBundleCachKey(
					bundleName, targetLocale))) {
				continue;
			}
			for (ClassLoader classLoader : getClassLoaders()) {
				ResourceBundle bundle = getBundle(bundleName, classLoader,
						targetLocale, key);
				if (bundle == null) {
					bundle = getBundle(bundleName, classLoader, defaultLocale,
							key);
				}
				if (bundle == null) {
	...
	private ResourceBundle getTargetBundle(String key, Locale targetLocale) {
		for (ResourceBundle cachedBundle : this.bundles) {
			if (cachedBundle.containsKey(key)
					&& ObjectUtils.equals(targetLocale,
							cachedBundle.getLocale())) {
				return cachedBundle;
			}
		}
		return null;
	 ...
	 * @return 如果源文件没有找到相应的键，则返回空消息。
	 */
	public synchronized String getMessage(String bundleName, String key,
			Object... params) {
	...
	public synchronized String getMessage(String bundleName, String key,
		...
		for (ClassLoader classLoader : getClassLoaders()) {
			ResourceBundle bundle = getBundle(bundleName, classLoader, locale,
					key);
			if (bundle == null) {
				continue;
			}
	 ...
	 * @return 如果源文件没有找到相应的键，则返回空消息。
	 */
	public synchronized String getMessage(Class<?> clazz, String key,
			Object... params) {
		return getMessage(XmgStringUtils.getPath(clazz, "Bundle"), key, params);
	}
	...
	public synchronized String getMessage(Class<?> clazz, String key,
			Locale locale, Object... params) {
		return getMessage(XmgStringUtils.getPath(clazz, "Bundle"), locale, key,
				params);
	}
